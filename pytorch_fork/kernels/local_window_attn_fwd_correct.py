"""Triton kernel for local window causal attention (forward pass) - CORRECT VERSION.

This version properly covers the full key range for all query rows by iterating
over key tiles with online softmax accumulation (FlashAttention-2 style).
"""
...
def local_window_attn_fwd_kernel(
    ...
    """
    Local window causal attention kernel (correct multi-tile version).

    Query at position i can attend to keys j where: j in [max(0, i-W+1), i]

    CRITICAL: This kernel iterates over key tiles to ensure FULL coverage of
    the valid key range for every query row, not just the first BLOCK_N keys.
    ...
    # Key range that this query tile needs
    ...
    # Iterate over key tiles: [k_min, k_min+BLOCK_N), [k_min+BLOCK_N, k_min+2*BLOCK_N), ...
    # BLOCK_M must be small enough (e.g., 1) that full key coverage fits in SRAM.
    # With BLOCK_M=1: need keys [q-W+1, q], span W keys, iterate ~W/BLOCK_N times.
    ...
    for k_tile_idx in tl.range(num_key_tiles):
        ...
        # This uses the identity: sum(exp(x)) = exp(m) * sum(exp(x-m))
        ...
        # out_new = (out_old * exp(m_old - m_new) + exp_scores @ v)
    ...
    # Finalize output
    # Handle inf case: if all entries were masked out, output is zero
    ...
    # Normalize output: out = out / sum(exp(scores))
    out = out / (s[:, None] + 1e-6)
